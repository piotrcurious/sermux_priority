...
void* client_control_thread(void *arg) {
    ...
    if (getsockopt(client_fd, SOL_SOCKET, SO_PEERCRED, &cred, &credlen) == 0) {
        have_cred = 1;
        log_msg("Control conn: peer pid=%d uid=%d gid=%d\n", cred.pid, cred.uid, cred.gid);
    }
#endif
...
#ifdef SO_PEERCRED
        uid_t peer_uid = have_cred ? cred.uid : (uid_t)-1;
        pid_t peer_pid = have_cred ? cred.pid : (pid_t)-1;
#else
...
#ifdef SO_PEERCRED
        pid_t peer_pid = have_cred ? cred.pid : supplied_pid;
#else
