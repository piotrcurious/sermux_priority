...
/*
 Extended preload that intercepts common file-open APIs on Linux:
 - open, open64, openat, creat
 - fopen, freopen (path != NULL case)
 - dup, dup2, dup3
 - fcntl (F_DUPFD, F_DUPFD_CLOEXEC)
 - close
 Includes:
 - thread-safe init (pthread_once)
 - robust socket comms to daemon (MSG_NOSIGNAL)
 - fd mapping bookkeeping
 - fallback behavior via env SERIALMUX_FALLBACK=1
 - target device specified by env SERIALMUX_DEVICE
*/
...
typedef struct {
    int fd;      // tracked fd in this process
    pid_t pid;   // process pid (for debugging)
} FDMapping;
...
static void add_mapping(int fd) {
    ...
    for (int i = 0; i < num_mapped; ++i) if (fd_map[i].fd == fd) { pthread_mutex_unlock(&map_lock); return; }
...
static void init_real_funcs_once(void) {
    ...
    // debug notice
    if (!target_device) debug_log("%s unset: not intercepting any device unless env provided", DAEMON_DEVICE_ENV);
}

/* talk to daemon and get PTY path */
static int connect_and_get_pty(const char *device, char *pty_path_buf, size_t buf_size) {
    int sock = socket(AF_UNIX, SOCK_STREAM, 0);
    if (sock < 0) {
        // perror suppressed to avoid noisy logs in preload
        return -1;
    }
    ...
    if (send_all(sock, msg, (size_t)len + 1) < 0) {
        close(sock);
        return -1;
    }
...
/* Helper: convert fopen mode string to flags for open() */
    ...
    if (connect_and_get_pty(path, pty_path, sizeof(pty_path)) == 0) {
        ...
        if (fd < 0) {
            if (allow_fallback) {
                ...
                add_mapping(fd2); // actually not mapping real device, but okay in fallback mode
...
/* freopen (path != NULL) - implement by closing old FILE* and opening new one */
FILE *freopen(const char *path, const char *mode_str, FILE *stream) {
    pthread_once(&init_once, init_real_funcs_once);
    if (!path) {
        // Not handling NULL path (reopen same file). Delegate to real_freopen if available.
...
int fcntl(int fd, int cmd, ...) {
    ...
    // Note: For commands requiring pointer args, this simplified wrapper may be insufficient.
