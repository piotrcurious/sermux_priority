...
//
// IMPROVED VERSION:
// This version fixes the critical flaw in 'handle_binary_request'.
// The original version used fixed-size buffers (sizeof(struct termios))
// and hard-coded logic for TCGETS/TCSETS, which is incorrect.
//
// This version uses the kernel's _IOC_ macros to generically handle
// *all* ioctl requests forwarded by the preload library.
//
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
#define MAX_IOCTL_BUF 8192 // Max size for a single ioctl arg
...
// --------------------------------------------------------------------------
// --- BEGIN CORRECTED BINARY REQUEST HANDLER ---
// --------------------------------------------------------------------------

/**
 * @brief Handles a binary SMIO request (ioctl, tcflush, etc.)
 *
 * This function is the counterpart to the preload library's
 * `send_request_and_get_response`. It reads the request and,
 * for `REQ_IOCTL`, it *generically* handles the request using
 * the kernel's `_IOC_` macros, fixing the original implementation's
 * hard-coded logic.
 *
 * Response format:
 * int32_t rc;
...
static int handle_binary_request(int ctrl_fd, uid_t peer_uid, pid_t peer_pid) {
    ...
    switch (req_type) {
        case REQ_IOCTL: {
            ...
            uint32_t arglen_from_client = 0;
            if (robust_read_all(ctrl_fd, &arglen_from_client, sizeof(arglen_from_client)) != (ssize_t)sizeof(arglen_from_client)) { err_no = EIO; break; }

            unsigned long request = (unsigned long)req64;
            size_t size = _IOC_SIZE(request);
            void *argp = NULL;

            // Allocate buffer for ioctl arg if needed
            if (size > 0) {
                // Limit to a reasonable size to prevent OOM
                if (size > MAX_IOCTL_BUF) {
                    log_msg("IOCTL req=0x%lx size %zu > max %d, rejecting.\n", request, size, MAX_IOCTL_BUF);
                    err_no = EINVAL;
                    rc = -1;
                    // We must send a reply
                ...
                argp = calloc(1, size);
                if (!argp) { err_no = ENOMEM; break; } // Will be handled by shared error path
            }

            // If preload sent data (WRITE ioctl), read it into our buffer
            if (argp && arglen_from_client > 0 && (_IOC_DIR(request) & _IOC_WRITE)) {
                // Sanity check: preload should send exactly 'size' bytes
                if (arglen_from_client != size) {
                     log_msg("Warning: IOCTL req=0x%lx client sent %u bytes, but size is %zu\n",
                             request, arglen_from_client, size);
                }
                size_t to_read = (arglen_from_client > size) ? size : arglen_from_client;

                if (robust_read_all(ctrl_fd, argp, to_read) != (ssize_t)to_read) {
                    err_no = EIO;
                    free(argp);
                    break;
                }
                
                // Drain any extra (malformed request)
                if (arglen_from_client > to_read) {
                    size_t remaining = arglen_from_client - to_read;
                    char drain[256];
                    while (remaining > 0) {
                         size_t d = (remaining > sizeof(drain)) ? sizeof(drain) : remaining;
                         if (robust_read_all(ctrl_fd, drain, d) != (ssize_t)d) {
                             err_no = EIO;
                             free(argp);
                             break; // outer switch
                         }
                    ...
                    if (err_no != 0) break; // outer switch
                }
            }
            ...
            if (ioctl_ret < 0) {
                ...
                rc = ioctl_ret;
                err_no = 0;
                // If it was a READ ioctl, set response data
                if (argp && (_IOC_DIR(request) & _IOC_READ)) {
                    out_arglen = (uint32_t)size;
                }
            }

            // Send response header
            if (robust_write(ctrl_fd, &rc, sizeof(rc)) < 0) { free(argp); return -1; }
            if (robust_write(ctrl_fd, &err_no, sizeof(err_no)) < 0) { free(argp); return -1; }
            if (robust_write(ctrl_fd, &out_arglen, sizeof(out_arglen)) < 0) { free(argp); return -1; }

            // Send response data (if any)
            if (out_arglen > 0 && argp) {
                if (robust_write(ctrl_fd, argp, out_arglen) < 0) {
                    free(argp);
                    return -1;
                }
            }
            free(argp);
            return 0; // Request fully handled.
        }
    ...
    // send response header (for non-IOCTL cases)
    if (robust_write(ctrl_fd, &rc, sizeof(rc)) < 0) return -1;
    if (robust_write(ctrl_fd, &err_no, sizeof(err_no)) < 0) return -1;
    if (robust_write(ctrl_fd, &out_arglen, sizeof(out_arglen)) < 0) return -1;
    // (out_arglen is 0 for these cases, so no data follows)
    return 0;
}

// --------------------------------------------------------------------------
// --- END CORRECTED BINARY REQUEST HANDLER ---
// --------------------------------------------------------------------------
