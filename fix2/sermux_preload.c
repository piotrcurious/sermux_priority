...
//
// IMPROVED VERSION:
// This version fixes the critical flaw in the 'ioctl' wrapper.
// The original version used fixed-size buffers (sizeof(struct termios)),
// which is incorrect and fails for any other ioctl (e.g., TIOCMGET).
//
// This version uses the kernel's _IOC_SIZE and _IOC_DIR macros to:
// 1. Determine the true size of the argument structure.
// 2. Send data from the user's pointer *only if* it's an _IOC_WRITE request.
// 3. Allocate a temporary buffer to receive data *only if* it's an _IOC_READ request.
// 4. Copy the received data back to the user's pointer.
//
// This makes the ioctl forwarding generic and correct for all tty ioctls.
//
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
// --------------------------------------------------------------------------
// --- BEGIN CORRECTED IOCTL FUNCTION ---
// --------------------------------------------------------------------------

/**
 * @brief Intercepts ioctl calls.
 *
 * This wrapper is the core of the library's serial control logic.
 * It performs the following checks:
 * 1. Is the file descriptor `fd` one that we mapped during `open()`?
 * 2. Does this `fd` *still* refer to the character device we are
 * configured to intercept (checking `st_rdev`)?
 * 3. Is this `fd` a TTY?
 *
 * If all are true, it forwards the `ioctl` call to the daemon using the
 * binary protocol. This function correctly and generically handles *all*
 * ioctl request types by using the kernel's `_IOC_` macros:
 *
 * - `_IOC_SIZE(request)`: Finds the size of the data structure (e.g.,
 * `sizeof(struct termios)` for `TCGETS`, `sizeof(int)` for `TIOCMGET`).
 * - `_IOC_DIR(request)`: Finds the data direction.
 * - `_IOC_WRITE`: If set, we *send* the data from `argp` to the daemon.
 * - `_IOC_READ`: If set, we allocate a temporary buffer, receive data
 * from the daemon, and copy it back to the user's `argp`.
 *
 * This fixes the original implementation which incorrectly hard-coded
 * `sizeof(struct termios)` and failed for all other ioctl types.
 */
int ioctl(int fd, unsigned long request, ...) {
    ...
    // The key fix: We trust the _IOC_ macros to tell us the size and direction.
    size_t size = _IOC_SIZE(request);
    void *send_buf = NULL;
    uint32_t send_len = 0;
    void *recv_buf = NULL;
    uint32_t recv_capacity = 0;

    if (size > 0 && argp) {
        // We only send data if the ioctl involves writing
        if (_IOC_DIR(request) & _IOC_WRITE) {
            send_buf = argp;
            send_len = (uint32_t)size;
        }
        // We only expect to receive data if the ioctl involves reading
        if (_IOC_DIR(request) & _IOC_READ) {
            // Allocate a temporary buffer to receive the reply
            // Use calloc to zero it, just in case.
            recv_buf = calloc(1, size);
            if (!recv_buf) {
                errno = ENOMEM;
                return -1;
            }
            recv_capacity = (uint32_t)size;
        }
    } else if (size > 0 && !argp) {
        // This is a sanity check. If size > 0, argp should not be NULL.
        // But if it is, pass NULL to the daemon (which will fail)
        // rather than crashing here.
        debug_log("Warning: ioctl req=0x%lx has size %zu but argp is NULL", request, size);
    }
    
    uint32_t got_len = 0;
    int daemon_errno = 0, out_rc = 0;

    if (send_request_and_get_response(REQ_IOCTL, (uint64_t)request,
                                     send_buf, send_len,
                                     &daemon_errno,
                                     recv_buf, recv_capacity,
                                     &got_len, &out_rc) < 0) {
        if (allow_fallback) {
            debug_log("daemon send/recv failed, falling back to passthrough ioctl fd=%d", fd);
            free(recv_buf);
            return passthrough_ioctl(fd, request, argp);
        }
        free(recv_buf);
    ...
    // If we received data into our temp buffer, copy it back to user's argp
    if (recv_buf && argp && got_len > 0) {
        // We copy at most 'size' bytes, which is what the user expects.
        size_t to_copy = (got_len > size) ? size : got_len;
        memcpy(argp, recv_buf, to_copy);
    }
